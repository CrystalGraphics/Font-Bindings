...
FontHandle *loadFontData(FreetypeHandle *library, const byte *data, int length) {
    if (!library || !data || length <= 0)
        return NULL;
    FontHandle *handle = new FontHandle;
    handle->fontData = new byte[length];
    std::memcpy(handle->fontData, data, length);
    FT_Error error = FT_New_Memory_Face(library->library, handle->fontData, length, 0, &handle->face);
    if (error) {
        delete[] handle->fontData;
        delete handle;
        return NULL;
    }
    handle->ownership = true;
